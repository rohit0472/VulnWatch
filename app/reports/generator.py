...
def generate_report(scan_data, username='User'):
    ...
    if isinstance(scanned_at, datetime):
        ...
        try:
            ...
            scan_date = (dt + IST).strftime('%Y-%m-%d %H:%M IST')
        except Exception:
            scan_date = scanned_at  # fallback if parsing fails
    ...
    SENSITIVE_KEYWORDS = [
        'admin', 'api', 'dev', 'staging', 'test', 'vpn',
        'login', 'secure', 'internal', 'portal', 'remote',
        'cpanel', 'webmail', 'mail', 'autodiscover', 'erp'
    ]
    ...
    risk_table = Table(risk_data, colWidths=[5.5 * cm, 6 * cm, 5 * cm])
    ...
    if missing_count == 0:
        header_note = "All security headers are properly configured."
    elif missing_count <= 2:
        header_note = f"Missing {missing_count} security header(s) may expose users to injection and downgrade attacks."
    else:
        header_note = f"{missing_count} security headers are missing, significantly increasing web attack exposure."
    ...
    story.append(Paragraph(
        ...
        f"{https_status}. {header_note} {cve_note} "
    ...
    stats_table = Table(summary_stats, colWidths=[6 * cm, 5 * cm, 5.5 * cm])
    ...
    if techs:
        tech_data = [['Technology', 'Version', 'Category', 'Risk Note']]
        for tech in techs:
            name = tech.get('name', '')
            version = tech.get('version') or 'Not detected'
            category = tech.get('category', 'unknown')
            if name in HIGH_RISK_TECHS:
                risk_note = 'Common attack target'
            elif name in MEDIUM_RISK_TECHS:
                risk_note = 'Monitor'
            else:
                risk_note = '-'
            tech_data.append([name, version, category, risk_note])

        tech_table = Table(tech_data, colWidths=[4.5 * cm, 3.5 * cm, 5 * cm, 3.5 * cm])
    ...
    story.append(Paragraph(
        ...
        'Possible CVEs are keyword-based and may include false positives.',
        styles['VMuted']
    ))
    story.append(Spacer(1, 0.2 * cm))
    ...
    counts_table = Table(counts_data, colWidths=[2.7 * cm] * 6)
    counts_table.setStyle(TableStyle([
        ...
        ('TEXTCOLOR', (1, 0), (1, -1), colors.HexColor('#b45309')),
    ...
    if verified:
        ...
        cve_data = [['CVE ID', 'Technology', 'Score', 'Severity', 'KEV']]
        for cve in verified:
            cve_data.append([
                cve.get('id', ''),
                cve.get('technology', ''),
        ...
        cve_table = Table(cve_data, colWidths=[4.5 * cm, 4 * cm, 2 * cm, 3.5 * cm, 2.5 * cm])
        ...
        story.append(Paragraph(
            'No verified CVEs found for detected technology versions.',
            styles['VMuted']
        ))
        story.append(Spacer(1, 0.2 * cm))
    ...
    if possible:
        story.append(Paragraph(
            f'<b>Top 10 Possible CVEs</b> (out of {len(possible)} keyword matches)',
            styles['VBody']
        ))
        pos_data = [['CVE ID', 'Technology', 'Score', 'Severity']]
        ...
        pos_table = Table(pos_data, colWidths=[4.5 * cm, 5.5 * cm, 2.5 * cm, 4 * cm])
    ...
    if subs:
        sub_data = [['Subdomain', 'Risk Flag']]
        for sub in subs:
            prefix = sub.split('.')[0]
            is_sensitive = any(k in prefix for k in SENSITIVE_KEYWORDS)
            sub_data.append([sub, 'Sensitive' if is_sensitive else '-'])

        sub_table = Table(sub_data, colWidths=[13.5 * cm, 3 * cm])
    ...
    if verified_cves > 0:
        high_priority.append(
            f'Patch or update technologies with {verified_cves} verified CVE(s).'
        )
    ...
    rec_table(medium_priority, colors.HexColor('#b45309'), 'Medium Priority')
